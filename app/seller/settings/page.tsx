...
export default function SettingsPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCompanyForm({
          ...
          ntn_number: data.company.ntn_number || '',
          gst_number: data.company.gst_number || '',
          fbr_token: data.company.fbr_token || '',
        ...
        setWhatsappForm({
          whatsapp_number: data.company.whatsapp_number || '',
          whatsapp_enabled: data.company.whatsapp_enabled || false,
          whatsapp_message_template: data.company.whatsapp_message_template || 
            'Hello {customer_name},\n\nYour invoice {invoice_number} for Rs. {total_amount} is ready.\n\nInvoice Date: {invoice_date}\nAmount: Rs. {total_amount}\n\nThank you for your business!\n\n{company_name}',
          whatsapp_api_mode: data.company.whatsapp_api_mode || 'link',
          whatsapp_phone_number_id: data.company.whatsapp_phone_number_id || '',
          whatsapp_access_token: data.company.whatsapp_access_token || '',
          whatsapp_business_account_id: data.company.whatsapp_business_account_id || '',
        });

        setEmailForm({
          email_enabled: data.company.email_enabled || false,
          smtp_host: data.company.smtp_host || '',
          smtp_port: data.company.smtp_port || 587,
          smtp_secure: data.company.smtp_secure || 'tls',
          smtp_user: data.company.smtp_user || '',
          smtp_password: data.company.smtp_password || '',
          smtp_from_email: data.company.smtp_from_email || '',
          smtp_from_name: data.company.smtp_from_name || data.company.name || '',
          email_subject_template: data.company.email_subject_template || 'Invoice {invoice_number} from {company_name}',
          email_body_template: data.company.email_body_template || 
        ...
        setSettingsForm({
          ...
          invoice_counter: data.settings.invoice_counter || 1,
          default_hs_code: data.settings.default_hs_code || '',
          default_sales_tax_rate: data.settings.default_sales_tax_rate || 18.0,
          default_further_tax_rate: data.settings.default_further_tax_rate || 0.0,
          invoice_template: data.settings.invoice_template || 'modern',
        });
      }
    } catch (error) {
  ...
  const handleSaveCompany = async (e: React.FormEvent) => {
    ...
    try {
      // Validate NTN format if provided (excluding dashes)
      const ntnWithoutDashes = companyForm.ntn_number.replace(/-/g, '');
      // if (companyForm.ntn_number && ntnWithoutDashes.length !== 7 && ntnWithoutDashes.length !== 13) {
          ...
          {activeTab === 'company' && (
            <form onSubmit={handleSaveCompany}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                onChange={(e) => {
  const value = e.target.value.replace(/[^0-9-]/g, ''); // allow digits and dashes
  setCompanyForm({ ...companyForm, ntn_number: value });
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Enter 7-digit NTN or 13-digit CNIC (dashes allowed). Current: {companyForm.ntn_number.replace(/-/g, '').length} digits
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div
                          ...
                          className={`cursor-pointer border-2 rounded-lg p-4 transition-all ${
                            whatsappForm.whatsapp_api_mode === 'link'
                        ...
                        <div
                          ...
                          className={`cursor-pointer border-2 rounded-lg p-4 transition-all ${
                            whatsappForm.whatsapp_api_mode === 'api'
                  ...
                  >
                    <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex justify-between items-center">
                      <h3 className="text-xl font-bold text-gray-900">
                        {templates.find((t) => t.template_key === selectedTemplatePreview)?.name} - Full Preview
                      </h3>
                      <button
                        onClick={() => setSelectedTemplatePreview(null)}
